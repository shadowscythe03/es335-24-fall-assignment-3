...
if activation_function == 'relu':
    ...
    pred_model.load_state_dict(torch.load(f"models_1k/model1k_{embedding_dim}_{context_length}_relu.pth", map_location=device))
...
if activation_function == 'tanh':
    ...
    # model_64_15_relu = NextWord(15, len(Vocabulary), 64, 1024).to(device)
    pred_model = NextWord(context_length, len(Vocabulary), embedding_dim, 1024).to(device)
    pred_model.load_state_dict(torch.load(f"models_1k/model1k_{embedding_dim}_{context_length}_tanh.pth", map_location=device))
